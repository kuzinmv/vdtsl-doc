...
4. При старте nodejs контейнера производятся следующие операции
   ...
   * 4.1. Загрузка метаданных 3х регионов (Омск, Санкт-Петербург, Краснодар)
   * 4.2. Загрузка 15-20 моделей (1..5 перекрестка) по каждому региону. Загрузка всех моделей может занять некоторое время 1-5 мин.
